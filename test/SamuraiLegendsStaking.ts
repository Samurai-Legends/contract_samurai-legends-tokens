...
`)
  ...
  it('Should let user claim 25% finished pendings', async function () {
    ...
    await expect(staking.connect(users[0]).claim(0)).to.emit(staking, 'PendingUpdated')
    const pending = await staking.userPending(users[0].address, 0)
  ...
  it('Should let user withdraw all after 10 weeks', async function () {
    ...
    const pending = await staking.userPending(users[0].address, 0)
    expect(await pending.claimedAmount).to.equal(0)
    expect(await pending.fullAmount).to.be.closeTo(userStake, amount(1).toNumber())
  })

  it('Should let user claim 100% finished pending', async function () {
    await toFuture('4 weeks')

    await expect(staking.connect(users[0]).claim(0)).to.emit(staking, 'Claimed')
  ...
  it('Should let the owner update reward duration and pending period', async function () {
    ...
    expect(await staking.userClaimablePendingPercentage(users[0].address, 0)).to.equal(
      amount(100),
    )
  })
  ...
  it('Should let the owner pause/unpause staking', async function () {
    ...
    await expect(staking.connect(users[0]).stake(amount(10_000))).to.be.reverted
  ...
  it('Should verify the smg total supply', async function () {
    ...
    for (const user of users) {
      await smg.connect(user).approve(user.address, await smg.balanceOf(user.address))
      await smg
        .connect(user)
        .transferFrom(user.address, owner.address, await smg.balanceOf(user.address))
  ...
  it('Should let users with shares 40% 30% 15% 10% 5% to stake', async function () {
    ...
    await expect(staking.addReward(amount(100_000))).to.emit(staking, 'RewardAdded')
    ...
    for (let i = 0; i < amounts.length; i++) {
      ...
      await expect(staking.connect(user).stake(_amount)).to.emit(staking, 'Staked')
  ...
  it('Should let users with shares 40% 30% 15% 10% 5% to withdrawAll', async function () {
    ...
    for (let i = 0; i < amounts.length; i++) {
      ...
      expect(pending.fullAmount).to.be.closeTo(_amount.mul(2), amount(1).toNumber())
      expect(await staking.userClaimablePendingPercentage(user.address, 0)).to.equal(
        amount(100),
      )
