...
describe("Koku", function () {
  ...
  async function getMintableTokens(
    ...
    const a = mintableTokensPerSecond.mul(
      (await getTimestamp()) - lastTimeMintedAt
    );
...
`);
  ...
  it("Should check admin roles", async function () {
    expect(await koku.hasRole(DEFAULT_ADMIN_ROLE, owner.address)).to.equal(
      true
    );
  ...
  it("Should let owner send 1k $KOKU to every account", async function () {
    ...
    await expect(koku.transfer(koku.address, amount(1_000))).to.emit(
      koku,
      "Transfer"
    );
  ...
  it("Should let owner add pair1 and pair2", async function () {
    await expect(koku.connect(users[0]).setPair(pair1.address, true)).to.be
      .reverted;

    // Add pair1
    await expect(koku.setPair(pair1.address, true)).to.emit(koku, "PairAdded");
    expect(await koku.isPair(pair1.address)).to.equal(true);
    await expect(koku.setPair(pair1.address, false)).to.emit(
      koku,
      "PairRemoved"
    );
  ...
  it("Shouldn't tax Owner -> Koku", async function () {
    ...
    await expect(koku.transfer(koku.address, amount(100))).to.emit(
      koku,
      "Transfer"
    );
  ...
  it("Shouldn't tax Owner -> Pair", async function () {
    ...
    await expect(koku.transfer(pair1.address, amount(100))).to.emit(
      koku,
      "Transfer"
    );
  ...
  it("Shouldn't tax Pair -> Owner", async function () {
    ...
    await expect(
      koku.connect(pair1).transfer(owner.address, amount(100))
    ).to.emit(koku, "Transfer");
  ...
  it("Shouldn't tax Owner -> User", async function () {
    ...
    await expect(koku.transfer(users[0].address, amount(100))).to.emit(
      koku,
      "Transfer"
    );
  ...
  it("Shouldn't tax User -> Owner", async function () {
    ...
    await expect(
      koku.connect(users[0]).transfer(owner.address, amount(100))
    ).to.emit(koku, "Transfer");
  ...
  it("Shouldn't tax Pair -> Koku", async function () {
    ...
    await expect(
      koku.connect(pair1).transfer(koku.address, amount(100))
    ).to.emit(koku, "Transfer");
  ...
  it("Shouldn't tax User -> Koku", async function () {
    ...
    await expect(
      koku.connect(users[0]).transfer(koku.address, amount(100))
    ).to.emit(koku, "Transfer");
  ...
  it("Shouldn't tax User -> User", async function () {
    const oldBalances = await balances(
      koku,
      users[0].address,
      users[1].address
    );
    await expect(
      koku.connect(users[0]).transfer(users[1].address, amount(100))
    ).to.emit(koku, "Transfer");
    const newBalances = await balances(
      koku,
      users[0].address,
      users[1].address
    );
  ...
  it("Should tax Pair -> User", async function () {
    ...
    await expect(
      koku.connect(pair1).transfer(users[0].address, amount(100))
    ).to.emit(koku, "Transfer");
  ...
  it("Should tax User -> Pair", async function () {
    ...
    await expect(
      koku.connect(users[0]).transfer(pair1.address, amount(100))
    ).to.emit(koku, "Transfer");
  ...
  it("Should tax Pair -> Pair", async function () {
    ...
    await expect(
      koku.connect(pair1).transfer(pair2.address, amount(100))
    ).to.emit(koku, "Transfer");
  ...
  it("Should let owner add/revoke MINTER_ROLE to game account", async function () {
    ...
    await expect(koku.grantRole(MINTER_ROLE, game.address)).to.emit(
      koku,
      "RoleGranted"
    );
    expect(await koku.hasRole(MINTER_ROLE, game.address)).to.equal(true);

    await expect(koku.revokeRole(MINTER_ROLE, game.address)).to.emit(
      koku,
      "RoleRevoked"
    );
    expect(await koku.hasRole(MINTER_ROLE, game.address)).to.equal(false);
  ...
  it("Should let owner make a special mint", async function () {
    ...
    await expect(
      koku.connect(users[0]).specialMint(hardcap)
    ).to.be.revertedWith(
    ...
    await expect(koku.specialMint(hardcap)).to.emit(
      koku,
      "AdminBalanceIncremented"
    );
    expect(await koku.totalSupply()).to.equal(totalSupply.add(hardcap));
    expect(await koku.balanceOf(owner.address)).to.equal(
      ownerBalance.add(hardcap)
    );
  ...
  it("Should check admin mintable tokens limit", async function () {
    ...
    await expect(
      koku.specialMint(mintableTokens.add(amount(1)))
    ).to.be.revertedWith("amount exceeds the mintable tokens amount.");
